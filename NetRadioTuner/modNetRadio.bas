...
'
' Other sources: RadioTunerfrm.frm & clsFileIo.cls
'
...
Public recdir As String
Public wholefil As Boolean
...
Public Sub Error_(ByVal es As String)
    Call MessageBox(RadioTunerfrm.hwnd, es & vbCrLf & vbCrLf & "error code: " & BASS_ErrorGetCode, "Error", vbExclamation)
End Sub
...
Sub DoMeta()
    ...
    If ((Mid(tmpMeta, 1, 13) = "StreamTitle='")) Then
        ...
        RadioTunerfrm.lblSong.Caption = TmpNameHold
               
         If wholefil = True Then
         DlOutput = recdir
         Else
            ...
            End If
         DlOutput = recdir & "\" & RemoveSpecialChar(Mid(p, 1, InStr(p, ";") - 2)) & ".mp3"
         End If
...
Public Sub SUBDOWNLOADPROC(ByVal buffer As Long, ByVal length As Long, ByVal user As Long)
    If (buffer And length = 0) Then
        RadioTunerfrm.lblBPS.Caption = VBStrFromAnsiPtr(buffer) ' display connection status
        Exit Sub
    End If
